...
export async function POST(request: Request) {
    ...
    const isDev = process.env.IS_DEV === 'true'; // Ensure your .env.local sets IS_DEV=true

    // Check if both conditions are met; otherwise, log and return early
    if (!(isDev && isWindows)) {
        console.log("Please use dev env to download pdf");
        return NextResponse.json({ message: "Please use dev env to download pdf" }, { status: 400 });
    }
